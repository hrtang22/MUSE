...
def train_epoch(epoch, args, model, train_dataloader, test_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    global logger
    global best_score
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and local_rank == 0:
                ...
                start_time = time.time()
            if global_step % (log_step*1) == 0 and local_rank == 0:
                R1 = eval_epoch(args, model, test_dataloader, device, n_gpu)
                if best_score <= R1:
                    best_score = R1
                logger.info("The best the R1 is: {:.4f}".format(best_score))
                model.train()
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    with torch.no_grad():
        batch_list_t = []
        batch_list_v = []
        batch_sequence_output_list, batch_visual_output_list, batch_seq_features_list = [], [], []
        total_video_num = 0
        batch_raw_image_list = []
        batch_video_ids = []
    ...
    if multi_sentence_:
        ...
        if print_matches:
            video_ids = []
            for vids in batch_video_ids:
                for vi in vids:
                    video_ids.append(vi)
            with open("/home/thr/code/S3/CLIP4Clip_mamba/matches_result/Mamba_result.txt", "w") as f:
                for idx, vi in enumerate(sim_matrix.argmax(axis=1)):
                    f.write(str((video_ids[idx], video_ids[vi])) + '\n')
...
def main():
    ...
    if args.do_train:
        ...
        for epoch in range(resumed_epoch, args.epochs):
            ...
            tr_loss, global_step = train_epoch(epoch, args, model, train_dataloader, val_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=args.local_rank)
            if args.local_rank == 0:
